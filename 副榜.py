...
def make_image_single(row: list) -> Image.Image:
    ...
    if row[header['上次']].upper() == 'NEW':
        ...
        brush.text(
            (config['lastrank_x'] - font_last_rank.getsize('▲ 上次：' + row[header['上次']])[0] // 2, config['lastrank_y']),
    ...
    elif row[header['上次']].upper().startswith('HOT') or int(row[header['名次']].replace(',', '')) > int(
            row[header['上次']].replace(',', '')):
        # ▼ 上次：xx
        brush.text((config['rank_x'] - font_num_rank.getsize(row[header['名次']])[0] // 2, config['rank_y']),
    ...
    if row[header['上次']] != 'NEW' and row[header['上次']] != '——':
        brush.text(
            (config['LastPt_x'] - font_last_pt.getsize('Last Pt. ' + row[header['Last Pt']])[0], config['LastPt_y']),
    ...
    # 头像图
    try:
        icon = Image.open('side_cover/uid' + str(row[header['mid']]) + '-' + row[header['up主']] + '.jpg').convert('RGBA')
...
rank_new = 0
for row in out_list:
    if row[header['名次']].startswith('HOT'):
        num_HOT += 1
    else:
        break
...
for row in out_list:
    ...
    if row[header['长期']] != '':
        if mode == 1:
            row[header['长期']] = row[header['长期']].zfill(2) + r' / 10'
        elif mode == 2:
            row[header['长期']] = row[header['长期']] + r' / 5'
        elif mode == 3:
            row[header['长期']] = row[header['长期']].zfill(2) + r' / 10'
